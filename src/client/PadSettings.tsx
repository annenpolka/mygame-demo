  ...
  return (
    <div className="modal-backdrop pad-backdrop">
      ...
      >
        ...
        <p className="muted">
          右トリガーで行動開始、左トリガーで後続取消。対象選択中の決定ボタンは基本技、補助・予約の選択中はその項目を決定。主力技と防御はどの操作先でも使えます。View／Share／−で対象・補助・予約を順に選び、補助内は左右で表示、上下で項目を選択します。戻る操作なしで全操作先へ移れます。左スティックは駒の配置に沿って移動。右スティックの左で味方、右で敵へ切り替えます。十字キーの左右で後列・前列へ移動、上でオプティマ、下で武器変更。R3でスローを切り替えます。右スティックは選び直す側を指定し、攻撃と支援の対象は同時に保持します。十字キー選択の配置では移動・装備変更を補助メニューから使えます。
        </p>
        <label className="pad-family">
          選択の操作方式
          <select
